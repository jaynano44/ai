...
SELECT EMPNO AS "사 번", ENAME AS "이 름", SAL AS "급여", JOB AS "직책" FROM EMP; -- 열이름에 별칭을 두는 경우 EX. 열이름 AS "별칭"
SELECT EMPNO,ENAME,SAL,JOB FROM EMP;
SELECT EMPNO "사 번", ENAME  "이 름", SAL  "급여", JOB   FROM EMP; -- 열이름에 별칭을 두는 경우 EX. 열이름  "별칭"
SELECT EMPNO "사 번", ENAME  이름, SAL  급여, JOB   FROM EMP; -- 별칭에 SPACE가 없는 경우 따옴표 생략가능 EX. 열이름  별칭

-- 3. 특정행 출력 :WHERE절(조건절) --  비교연산자 : 같다(=),다르다(!=,^=,<>),>,>=,<,<=,...
SELECT EMPNO 사번, ENAME 이름, SAL 급여 FROM EMP WHERE SAL=3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL!=3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL^=3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL<>3000;

        -- 비교연산자는 수자,문자,날짜형 모두 가능
        ...
        -- ㄴSQL문은 대소문자 구별없음. 데이터는 대소문자 구별
        -- ex4. 이름(ENAME)이 SCOTT인 직원의 모든 데이터(모든 필드)
        SELECT * FROM EMP WHERE ENAME='SCOTT';
       -- SELECT * FROM EMP WHERE NAME='SCOTT';
       
-- 4. WHERE절 (조건절)에 논리연산자 : AND  OR NOT
    -- ex1. 급여(SAL)가 2000인 이상 3000이하인 직원의 모든 필드
...
SELECT EMPNO , ENAME, SAL "예전 월급", SAL*1.1 "현재월급"  FROM EMP;
    ...
    -- SELECT  ENAME 이름 , SAL 월급 ,COMM 상여, SAL*12+COMM 연봉 FROM EMP;    -- NULL있을것 같은 필드 NVL
    SELECT  ENAME 이름 , SAL 월급 ,COMM 상여, SAL*12+NVL(COMM,0) 연봉 FROM EMP; 
    
    -- ex3. 모든 사원의 ENAME, MGR(상사사번)을 출력 - 상사사번이 없으면, 'CEO'
    SELECT ENAME,NVL(TO_CHAR(MGR),'CEO') MGR FROM EMP;   --필드명과 대체값은 타입이 일치
    DESC EMP;
...
SELECT EMPNO 사번, ENAME 이름, SAL 급여,JOB 업무, DEPTNO 부서코드
    FROM EMP
...
-- (1) 필드명 BETWEEN A AND B  : 필드명이 A부터 B까지(A,B포함. A<=B)
    ...
    -- ex1-1. SAL이 1500 미만 300초과 (ex1의 반대)
    SELECT * FROM EMP 
    ...
    SELECT * FROM EMP 
        WHERE TO_CHAR(HIREDATE,'RR/MM/DD') BETWEEN '81/03/01' AND '81/05/31'  -- 날짜주의
--(2) 필드명 IN(값1,값2,...,값N)
...
-- (3) 필드명 LIKE '패턴' : %(0글자이상),_(한글자)를 포함하는 패턴
    ...
    DESC EMP;
        -- 이름에 %가 들어간 데이터 INSERT  : 실행전에는 오라클에 삽입되지 않음.
        INSERT INTO EMP VALUES (9999,'홍%동',NULL,NULL,NULL,9000,9000,40);
    SELECT * FROM EMP;
    SELECT * FROM EMP WHERE ENAME LIKE '%\%%' ESCAPE '\';  --  \ 대신 아무거나 다됨
        --  실행 전에 INSERT한  데이터 취소 : ROLLBACK 
...
-- (4) 필드명 IS NULL :필드명이 널인지를 검색할 때
    ...
    -- ex. COMM(상여)을 받는 사원(COMM!=0 AND COMM이 NULL 아님)
