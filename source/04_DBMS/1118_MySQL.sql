...
-- DDL (제약조건 FK,시퀀스없음)
-- DML (and:&&,or:||,연결연산자대신concat	사용,outer join)
...
-- 기본 데이터베이스들(스키마)
show databases;
create database davdb; -- 새로운 데이터베이스(스키마)  devdb 생성
use davdb;             -- devdb 데이터베이스로 들어감
...
문자 : char(n) => 고정자리라 잘안씀, text,mediumtext(16M),longtext(4GB)
...
create table major(
	mcode int primary key auto_increment, -- auto_increment필드(반드시 int타입)
    mname varchar(10),
    moffice varchar(10)
);
select * from major;
...
insert into student values(104,'김길동',3);

--  FK(외래키) 제약조건은 아래에 기술해야함..  위와 같이 하면 암됨.
...
select sno,sname,s.mcode,mname,moffice
	from student s right outer join   major m       -- from student s, major m  오른쪽이 더 많은 경우
    on s.mcode = m.mcode;      				 -- where s.mcode = m.mcode;
...
-- 6. 사번, 이름, 부서명
select pno,pname,dname as '부서명' 
	FROM personal p ,division d
    where p.dno = d.dno;
-- 7. 사번, 이름, 상사이름
select p.pno,p.pname,m.pname as '상사명' 
...
select p.pno, p.pname, ifnull(m.pname, 'CEO') as '상사명' , d.dname 
	from personal p 
...
select sysdate() ;  -- select sysdate() from dual;  oracle 지금 현재시간
